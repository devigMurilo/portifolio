...
function LiquidButton({
  ...
  /** As camadas de vidro + o conteúdo, na ordem em que precisam ser pintados. */
  const glass = (content: React.ReactNode) => (
    <>
      ...
      {/*
        No original este wrapper é um `<div>` block, então `items-center gap-2` do
        botão não alcança o conteúdo — as camadas de vidro são absolutas e saem do
        fluxo, sobrando um único filho em fluxo. Resultado: ícone + rótulo empilham
        em duas linhas. `inline-flex` devolve o alinhamento em uma linha.
      */}
      <div className="pointer-events-none z-10 inline-flex items-center gap-2">{content}</div>
      <GlassFilter />
    </>
  )

  /*
   * `asChild` no componente original envia as camadas de vidro como irmãs para o
   * Slot, e o Slot do Radix aceita um único filho (`React.Children.only`) — então
   * a prop quebrava em tempo de execução. Aqui o filho do consumidor (por exemplo
   * um `<a>`) vira a raiz e recebe as camadas dentro dele.
   */
  if (asChild && React.isValidElement<{ children?: React.ReactNode }>(children)) {
    return (
      <Slot data-slot="button" className={classes} {...props}>
        {React.cloneElement(children, undefined, glass(children.props.children))}
      </Slot>
    )
  }
